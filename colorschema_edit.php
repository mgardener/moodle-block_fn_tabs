...
if ($duplicate) {
    ...
    $schemaid = $DB->insert_record('format_ned_tabs_color', $schema);
    redirect(new moodle_url('/course/format/ned_tabs/colorschema_edit.php', array('courseid' => $courseid, 'edit' => $schemaid)));
}

$PAGE->https_required();

$thispageurl = new moodle_url('/course/format/ned_tabs/colorschema_edit.php',
...
$name = get_string('addedit', 'format_ned_tabs');
$title = get_string('addedit', 'format_ned_tabs');
...
if ($edit) {
    if (!$toform = $DB->get_record('format_ned_tabs_color', array('id' => $edit, 'predefined' => 0))) {
        redirect(new moodle_url('/course/format/ned_tabs/colorschema.php', array('courseid' => $courseid)));
    }
}
...
if ($mform->is_cancelled()) {
    ...
    if ($add) {
        ...
        $rec->id = $DB->insert_record('format_ned_tabs_color', $rec);
        redirect(new moodle_url('/course/format/ned_tabs/colorschema.php', array('courseid' => $courseid)),
            get_string('successful', 'format_ned_tabs'), 0);
    } else {
        $rec->id = $fromform->edit;
        $rec->timemodified = time();
        $DB->update_record('format_ned_tabs_color', $rec);
        redirect(new moodle_url('/course/format/ned_tabs/colorschema.php', array('courseid' => $courseid)),
            get_string('successful', 'format_ned_tabs'), 0);
    }
    exit;
